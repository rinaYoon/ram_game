...
// function areaSlide() {
// 	$( '.service__web [class^="area-"] .swiper-container' ).each( function ( i ) {
// 		new Swiper( $( this )[ 0 ] ,{
// 			loop : true,
// 			slidesPerView: 1,
// 			centeredSlides: true,
// 			watchOverflow: true,
// 			pagination: {
// 				el: $( '.service__web [class^="area-"] .swiper-pagination' )[ i ],
// 				clickable: true,
// 			},
// 			// navigation: {
// 			// 	nextEl: $( '.service__web [class^="area-"] .button-next' )[ i ] ,
// 			// 	prevEl: $( '.service__web [class^="area-"] .button-prev' )[ i ] ,
// 			// },
// 		});
// 	})
// }
	...
	areaFirst.forEach(function (slot, index){
		...
		if (nowTime >= slot.startTime && slot.endTime > nowTime) {
			if (!$('#' + id).length) {
				...
				var item01Title = $('.area-discount-first .slot-coupon01 .coupon-wrap dt');
				var item01Desc = $('.area-discount-first .slot-coupon01 .coupon-wrap dd');

				var itemsNotice = $('.area-discount-first .division-first .first-notice .notice-bundle');

				$(".notice-bundle").attr("id", id);
				...
				itemsNotice.css("background-image","url(" + slot.noticeImageUrl + ")");
				$(slot.noticeDesc).appendTo(itemsNotice);

				// 2023-10-26 해당 기간에만 area와 nav메뉴 노출
				$('.area-discount-first').show();
				$('.event-nav .nav-memu01').after( '<li class="items-list__item nav-memu02"><a href="#area-first" class="items-list__link">웰컴백 쿠폰</a></li>' );
			}
		} else {
			$('#' + id).remove(slot.item01ImageUrl);
		}
	});
	...
	areaThisWeek.forEach(function (slot, index){
		...
		if (nowTime >= slot.startTime && slot.endTime > nowTime) {
			if (!$('#' + id).length) {
				...
				var imageName = $('.area-discount-thisweek .division-thisweek');
				...
				$('.event-nav .nav-memu04').before( '<li class="items-list__item nav-memu03"><a href="#area-thisweek" class="items-list__link">카드 즉시할인</a></li>' );
	...
	areaCardNotice.forEach(function (slot, index){
		...
		if (nowTime >= slot.startTime && slot.endTime > nowTime) {
			if (!$('#' + id).length) {
				...
				$('.area-card-notice').show();// 2023-10-26
			}
		} else {
			$('#' + id).remove();
		}
